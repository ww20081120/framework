...
public class JsxGeneratorTool extends AbstractBaseTool<JsxGeneratorTool.JsxInput> {
	...
	/**
	 * Internal input class for defining input parameters of Vue component generator
	 * tool
	 */
		...
		try {
			...
			case "generate_vue" -> {
				...
				if (componentType == null || componentType.trim().isEmpty()) {
					yield new ToolExecuteResult(createDetailedErrorMessage("generate_vue", "component_type", input));
				}
				...
				LoggerUtil.info("Generating Vue SFC with componentType: {0}, componentData keys: {1}", componentType,
						componentData.keySet());
				String vueSfcCode = vueGeneratorService.generateVueSFC(componentType, componentData);
				yield new ToolExecuteResult("Successfully generated Vue SFC code for component type '" + componentType
						+ "':\n" + vueSfcCode);
			}
			case "apply_template" -> {
				...
				if (templateName == null || templateName.trim().isEmpty()) {
					yield new ToolExecuteResult(createDetailedErrorMessage("apply_template", "template_name", input));
				}
				...
				yield new ToolExecuteResult("Successfully applied template '" + templateName + "':\n" + generatedCode);
			}
			case "save" -> {
			...
			case "preview" -> {
				...
				LoggerUtil.info("Generating preview for Vue component: {0}, dependencies: {1}", filePath, dependencies);
