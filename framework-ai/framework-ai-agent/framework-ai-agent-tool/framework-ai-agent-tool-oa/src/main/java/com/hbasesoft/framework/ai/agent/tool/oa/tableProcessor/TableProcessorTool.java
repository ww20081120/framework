...
public class TableProcessorTool extends AbstractBaseTool<TableProcessorTool.TableInput> {
	...
	public TableProcessorTool(ITableProcessingService tableProcessingService) {
	...
	public ToolExecuteResult run(TableInput input) {
		LoggerUtil.info("TableProcessorTool input: action={0}, filePath={1}", input.getAction(), input.getFilePath());
		try {
	...
	public String getCurrentToolStateString() {
		...
		try {
			...
			return String.format("""
					...
					%s
					""", workingDir.toString(),
					tableProcessingService.getLastOperationResult(planId).isEmpty() ? "No operation performed yet"
							: tableProcessingService.getLastOperationResult(planId));
		} catch (Exception e) {
			return String.format("""
					Current Table Processing State:
	...
	public void cleanup(String planId) {
		if (planId != null) {
			LoggerUtil.info("Cleaning up table processing resources for plan: {0}", planId);
