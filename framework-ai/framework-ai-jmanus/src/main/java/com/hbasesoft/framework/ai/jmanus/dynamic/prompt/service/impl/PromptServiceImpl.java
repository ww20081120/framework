...
import com.hbasesoft.framework.ai.jmanus.dynamic.prompt.dao.PromptDao;
import com.hbasesoft.framework.ai.jmanus.dynamic.prompt.model.enums.PromptEnum;
import com.hbasesoft.framework.ai.jmanus.dynamic.prompt.model.po.PromptPo;
...
public class PromptServiceImpl implements PromptService {
	...
	public List<PromptVO> getAllByNamespace(String namespace) {
		List<PromptPo> entities;
		if ("default".equalsIgnoreCase(namespace)) {
			entities = promptRepository.queryByLambda(q -> q.or(q1 -> q1.eq(PromptPo::getNamespace, namespace)
					.isNull(PromptPo::getNamespace).eq(PromptPo::getNamespace, "")));
		} else {
			entities = promptRepository.queryByLambda(q -> q.eq(PromptPo::getNamespace, namespace));
		}
	...
	public PromptVO getById(Long id) {
		PromptPo entity = promptRepository.get(id);
	...
	public PromptVO getPromptByName(String promptName) {
		PromptPo entity = promptRepository
				.getByLambda(q -> q.eq(PromptPo::getNamespace, namespace).eq(PromptPo::getPromptName, promptName));
		if (entity == null) {
	...
	public PromptVO create(PromptVO promptVO) {
		...
		PromptPo prompt = promptRepository.getByLambda(q -> q.eq(PromptPo::getNamespace, promptVO.getNamespace())
				.eq(PromptPo::getPromptName, promptVO.getPromptName()));
		if (prompt != null) {
			LoggerUtil.error("Found Prompt is existed: promptName :{0} , namespace:{1}, type :{2}, String messageType:{3}",
		...
		PromptPo promptEntity = mapToPromptPo(promptVO);
	...
	public PromptVO update(PromptVO promptVO) {
		...
		PromptPo oldPrompt = promptRepository.get(promptVO.getId());
		...
		LoggerUtil.info("Successfully update new Prompt promptName :{0} , namespace:{1}, type :{2}, String messageType:{3}",
				promptEntity.getPromptName(), promptEntity.getNamespace(), promptEntity.getType(),
				promptEntity.getMessageType());
		return mapToPromptVO(promptEntity);
	}
	...
	public Message createSystemMessage(String promptName, Map<String, Object> variables) {
		PromptPo promptEntity = promptRepository
				.getByLambda(q -> q.eq(PromptPo::getNamespace, namespace).eq(PromptPo::getPromptName, promptName));
	...
	public void importAllPromptsFromLanguage(String language) {
		...
		for (PromptEnum promptEnum : PromptEnum.values()) {
			try {
				...
				LoggerUtil.error("Failed to reset prompt: {0} to language: {1}", promptEnum.getPromptName(), language, e);
			}
		}
	...
	private PromptVO mapToPromptVO(PromptPo entity) {
		PromptVO promptVO = new PromptVO();
		BeanUtils.copyProperties(entity, promptVO);
		return promptVO;
	}

	private PromptPo mapToPromptPo(PromptVO promptVO) {
		PromptPo entity = new PromptPo();
	...
	public void reinitializePrompts() {
		...
		List<PromptPo> allPrompts = promptRepository.queryAll();
		LoggerUtil.info("Found {0} prompts in total", allPrompts.size());

		int updatedCount = 0;
		int validCount = 0;

		for (PromptPo prompt : allPrompts) {
		...
		LoggerUtil.info("Prompt namespace correction completed. Summary: {0} prompts updated, {1} prompts already valid.",
