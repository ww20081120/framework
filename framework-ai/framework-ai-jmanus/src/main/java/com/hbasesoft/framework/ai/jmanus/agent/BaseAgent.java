 ...
 * tasks. 用于实现可执行多步骤任务的 AI 智能体的抽象基类。 This class provides the core functionality
 * for managing agent state, conversation flow, 该类提供了管理智能体状态、对话流程的核心功能， and
 * step-by-step execution of tasks. 以及任务的逐步执行功能。
 *
 * <p>
 * The agent supports a finite number of execution steps and includes mechanisms
 * for: 该智能体支持有限数量的执行步骤，并且包含以下机制：
 * <ul>
...
public abstract class BaseAgent {
	 ...
	 * name should reflect the main functionality or characteristics of the agent 3.
	 * The name should be unique for easy logging and debugging
	 *
	 * Example implementations: - ToolCallAgent returns "ToolCallAgent" -
	 * BrowserAgent returns "BrowserAgent"
	 * 
	 ...
	 * responsibilities and capabilities 3. Should explain how this agent differs
	 * from other agents
	 *
	 ...
	 * "Agent that implements alternating execution of reasoning and acting"
	 * 
	 ...
	 * and make decisions 3. Can recursively build prompt chains to form
	 * hierarchical thinking processes 4. Return the added system prompt message
	 * object
	 *
	 * Subclass implementation reference: 1. ReActAgent: Implement basic
	 * thinking-action loop prompts 2. ToolCallAgent: Add tool selection and
	 ...
	 * agent to perform the next step 2. The prompt should be based on the current
	 * execution state and context 3. The message should clearly guide the agent on
	 * what task to perform
	 *
	 * Subclass implementation reference: 1. ToolCallAgent: Return prompts related
	 * to tool selection and execution 2. ReActAgent: Return prompts related to
	 * reasoning or action decision
	 * 
	 ...
	 * agent's execution 2. Data can include: - Current execution state - Step
	 * information - Intermediate results - Configuration parameters 3. Data is set
	 * through setData() when run() is executed
	 *
