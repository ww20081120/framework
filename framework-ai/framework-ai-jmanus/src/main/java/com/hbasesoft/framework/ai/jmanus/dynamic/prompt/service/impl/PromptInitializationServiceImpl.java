...
public class PromptInitializationServiceImpl implements PromptInitializationService {
	...
	private void createPromptIfNotExists(String namespace, PromptEnum prompt, String language) {
		...
		PromptPo promptEntity = TransactionUtil.withSession((tm, ts) -> {
			return promptRepository.getByLambda(
					q -> q.eq(PromptPo::getNamespace, namespace).eq(PromptPo::getPromptName, prompt.getPromptName()));
		});

		if (promptEntity == null) {
			promptEntity = new PromptPo();
