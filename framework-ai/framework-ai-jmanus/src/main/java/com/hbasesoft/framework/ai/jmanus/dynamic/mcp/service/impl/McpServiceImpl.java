 ...
 ****************************************************************************************/
package com.hbasesoft.framework.ai.jmanus.dynamic.mcp.service.impl;
...
public class McpServiceImpl implements IMcpService {
	 ...
	 */
	@Override
	public List<McpConfigPo> saveMcpServers(String configJson) throws IOException {
		List<McpConfigPo> entityList = new ArrayList<>();
		...
		for (Map.Entry<String, McpServerConfig> entry : mcpServers.entrySet()) {
			...
			McpConfigPo mcpConfigEntity = mcpConfigDao
					.getByLambda(q -> q.eq(McpConfigPo::getMcpServerName, serverName));
			if (mcpConfigEntity == null) {
				mcpConfigEntity = new McpConfigPo();
	 ...
	 */
	@Override
	public McpConfigPo saveMcpServer(McpServerRequestVO requestVO) throws IOException {
		...
		if (requestVO.isUpdate()) {
			...
			McpConfigPo existingServer = mcpConfigDao
					.getByLambda(q -> q.eq(McpConfigPo::getMcpServerName, requestVO.getMcpServerName()));
	 ...
	 */
	@Override
	public List<McpConfigPo> getMcpServers() {
	 ...
	 */
	public Optional<McpConfigPo> findById(Long id) {
	...
	public boolean updateMcpServerStatus(Long id, McpConfigStatus status) {
		McpConfigPo entity = mcpConfigDao.get(id);
