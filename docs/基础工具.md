...
项目中经常能遇到启动时就需要加载某些功能的场景，例如项目启动时需要检查数据库表有没有创建，没有创建则需要创建相应的数据。Spring默认已经支持了这种加载方式，但有些场景我们可能需要在Spring容器加载之前启动，我们提供了 `com.hbasesoft.framework.common.StartupListener` 接口来支撑这种场景。

> StartupListener.java

文件路径：`framework-common/framework-common-core/src/main/java/com/hbasesoft/framework/common/StartupListener.java`
...
public interface StartupListener {
    ...
    /**
     * 是否启用该监听器
     * 默认从配置文件读取：{类全名}.=true/false
     * 例如：com.example.MyListener.=false
     *
     ...
     * 定义多个Listener之间的执行顺序
     *
     * @return 加载顺序：FIRST（最先）、MIDDLE（中间）、LAST（最后）
     */
...
**步骤1**：编写业务代码实现 `StartupListener` 接口
...
**步骤2**：在项目的 `src/main/resources/META-INF/services` 目录下创建SPI配置文件

文件路径：`src/main/resources/META-INF/services/com.hbasesoft.framework.common.StartupListener`

文件内容：
```
com.example.listener.ExampleStartupListener
```

如果有多个实现类，每行一个：
```
com.example.listener.ExampleStartupListener
com.example.listener.AnotherStartupListener
```

**步骤3**（可选）：在 `application.yml` 中配置是否启用该监听器

```yaml
com:
  example:
    listener:
      ExampleStartupListener: false  # 禁用该监听器
```

#### 执行流程说明

框架通过 `Bootstrap` 类管理启动流程：

1. **Bootstrap.before()** 在Spring容器加载前执行：
   - 设置代理配置（http、https、ftp、socks）
   - 通过Java SPI加载所有 `StartupListener` 实现类
   - 按 `LoadOrder` 排序
   - 调用每个监听器的 `init()` 方法

2. **Bootstrap.after()** 在Spring容器加载完成后执行：
   - 调用每个监听器的 `complete()` 方法
   - 打印启动成功信息和访问地址

> Bootstrap.java

文件路径：`framework-common/framework-common-core/src/main/java/com/hbasesoft/framework/common/Bootstrap.java`
...
框架内置了多个 `StartupListener` 实现：

- `DatasourceStartupLinstener` - 数据源初始化
- `JobStartupLinstener` - 定时任务初始化
- `TxAnnotationMethodRegistStartupListener` - 事务注解方法注册
- `EventHandlerStartupLinstener` - 事件处理器初始化
