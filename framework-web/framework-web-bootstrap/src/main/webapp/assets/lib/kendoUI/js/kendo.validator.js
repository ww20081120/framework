        ...
        options: {
            ...
            rules: {
                ...
                unique: function (input) {
                    if (!hasAttribute(input, "unique")) {
                        return true;
                    }
                    var result = false, kendoUrl = kendo.attr("unique-url");
                    if(hasAttribute(input, kendoUrl)){
                        var _data = {}, kendoId = kendo.attr("unique-id"), kendoValue = kendo.attr("unique-value");
                        _data[input.attr("name")] = input.val();
                        if (hasAttribute(input, kendoId)) {
                            var relationId = input.attr(kendoId), relationObj = $("#" + relationId);
                            _data[relationObj.attr("name")] = relationObj.val();
                        } else if(hasAttribute(input, kendoValue)){
                            _data["relationId"] = input.attr(kendoValue);
                        }
                        result = $.ajax({
                            url: input.attr(kendoUrl),
                            data: _data,
                            async: false
                        }).responseText;
                    }
