...
import com.fccfc.framework.web.manager.service.common.ResourceService;
...
public class ResourceController extends AbstractController {
    ...
    public Object download(@RequestParam("mediaId") Integer resourceId,
        ...
        if ("Y".equals(attachment.getIsPicture()) && "1".equals(isThumb)
            ...
            file = new File(ConfigHelper.getString(ManagerConstant.CONFIG_ITEM_RESOURCE_PATH) + attachment.getThumbPath());
        }
        else {
            file = new File(ConfigHelper.getString(ManagerConstant.CONFIG_ITEM_RESOURCE_PATH) + attachment.getFilePath());
        }
    ...
    public Map<String, Object> upload(HttpServletRequest request) throws FrameworkException {
        ...
        File dir = new File(ConfigHelper.getString(ManagerConstant.CONFIG_ITEM_RESOURCE_PATH) + path);
        if (!dir.exists()) {
            dir.mkdirs();
        }
        ...
        try {
            ...
            for (Map.Entry<String, MultipartFile> fileEntry : fileMap.entrySet()) {
                ...
                if (isBase64) {
                    ...
                    IOUtil.copyFileFromInputStream(dir.getAbsolutePath() + fileName, file.getInputStream(), "text".equals(type) ? GlobalConstants.DEFAULT_CHARSET : null);
                }
                results.add(saveFile(file.getOriginalFilename(), contentType, currentDate, expDate, path + fileName, file.getSize()));
            }
